output$Age_range <- renderUI({
    ifelse(input$type == "Variances of the correlations", choices <- Age_ranges_Correlations_equalbins, choices <- Age_ranges_Correlations)
    selectInput(
    inputId = "age_range",
...
output$abs <- renderUI({
    if(input$type == "Correlations"){checkboxInput("abs", "Take the absolute values of the correlations", value = TRUE)}
})
...
type <- reactive({
    ...
    "Variances of the correlations" = "Correlations_var")
})

data <- reactive({
  ifelse(grepl("var", type()), rdRDS <- redRDS_equalbins, rdRDS <- redRDS)
  display <- ifelse(grepl("var", type()) & (grepl("vs", demo_group()) | grepl("VS", distance())), "display", "")
  data <- rdRDS("Correlations", type(), display, "flat", distance(), demo_group())
  if(input$abs){data <- abs(data)}
  if(input$significant)
  {
    significants <- rdRDS("Correlations", type(), "significances", distance(), demo_group())
...
data_histo <- reactive({
  ...
  data_histo <- rdRDS("Correlations","data_histo", type(), type3, type4, distance(), demo_group())
  data_histo
})

output$plot_cor_hg <- renderPlot({
    data <- data()
    mn <- min(data[,-1])
    mx <- max(data[,-1])
    binwidth = (mx - mn)/100
...
output$summaryplot_cor_hg <- renderPlot({
    ...
    #save figure
    if(generate_figures){ggsave("./figures/figure4.pdf", plot = p, device = "pdf", dpi = 600, height = 20, width = 40, units = "cm")}
    } else {
